...
#   VARIABLE: DATASET NAME TO BE DOWNLOAD:
...
def xml_clean_the_dataset_for_rails(dataset_name):
    ...
    for node in links:
        ...
        if this_artefact_contains_at_least_two_images(artefact_xml, 'DocIdentifier', not_raw_xml_file):
            ...
            art_location = find_single_element_value_in_xml(artefact_xml, 'LocLevel1', not_raw_xml_file)
            ...
            a.update({'dataset_name': xml_file_name_index, 'unique_identifier':unique_identifier, 'art_location': art_location})
            #print(art_location)
